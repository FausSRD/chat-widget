(function() {
    class LhChatWidget extends HTMLElement {
      connectedCallback() {
        // placeholder mientras llega la config
        this.innerHTML = `<div style="padding:10px;">Carregando chat...</div>`;
  
        // escuchar mensajes desde Wix
        window.addEventListener("message", (event) => {
          if (event.data.type === "LOAD_WIDGET") {
            this.loadChat(event.data.config);
          }
        });
      }
  
      loadChat(chatWidgetConfig) {
        const configSource = chatWidgetConfig || {}
        const config = {
